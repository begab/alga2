...
\begin{itemize}
	...
	\end{itemize}
	\end{itemize}
\begin{alertblock}<2->{Ha nem hajtunk végre {\rmfamily\scshape Sorbol-min} és 
{\rmfamily\scshape Töröl} műveleteket}
	Egy (fokszám alapján) rendezetlen ''binomiális 
	kupacot'' kapunk \\
	$\rightarrow \log{n}$ a kupacbeli csúcsok maximális fokszámának 
	felső korlátja
\end{alertblock}
\begin{block}<3>{{\rmfamily\scshape Sorbol-min} és {\rmfamily\scshape Töröl} 
műveletek végrehajtása esetén}
	$O(\log{n})$ (pontosabban 
	$\log_\phi{n}$\footnote{$\phi=\frac{1+\sqrt{5}}{2}$}) a kupacbeli csúcsok 
	maximális fokszámának felső korlátja $\rightarrow$ innen jön a 
	Fibonacci-kupac elnevezés
\end{block}
\end{frame}
