...
%	\begin{tabular}{@{}ll@{}}
	$n$ elemű kupacban hogy keresnénk meg a maximális elemet? \hfill 
	\only<2>{$O(1)$} \\
	És egy adott kulcs rákövetkezőjét? \hfill \only<2>{$O(n)$} \\
	Hogy egyesítenénk egy $n$ és egy $m$ kulcsból álló kupacot? 
	\hfill 
\only<2>{$O(n+m)$} \\
%	\end{tabular}
